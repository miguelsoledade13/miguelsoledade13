 <img width=100% src="https://capsule-render.vercel.app/api?type=waving&color=683086&height=180&section=header&text=Miguel+Soledade&fontSize=25&fontColor=ffffff&animation=twinkling&fontAlignY=35"/>
  Olá, eu sou Miguel Soledade Delurdes
<img src="https://readme-typing-svg.herokuapp.com/?color=00F7FF&size=24&center=true&vCenter=true&width=500&lines=Bem-vindo+ao+meu+perfil!;Desenvolvedor+em+formação;Sempre+aprendendo" />
> “Para alcançar o céu, é preciso confiar no destino… e aceitar tudo o que ele traz.” 🌀  
> *(Inspirado em JoJo’s Bizarre Adventure)*

  Sobre mim

Sou um entusiasta de tecnologia em constante evolução, sempre buscando aprimorar minhas habilidades e explorar novas áreas do desenvolvimento.

Tenho experiência inicial com:

- 🌐 HTML e CSS (estrutura de sites)
- ⚡ JavaScript (noções básicas)
- 💻 Sistemas Operacionais: Windows e Linux
- 🖥️ Virtualização com VirtualBox

---

<img src="https://komarev.com/ghpvc/?username=miguelsoledade13&color=blue" />

 🎓 Formação - Microcamp Informática

Concluí um curso completo de informática na **Microcamp**, onde adquiri uma base sólida em diversas áreas da tecnologia:

 🧠 Fundamentos de Informática
- Uso do computador (Windows, arquivos e pastas)
- Internet e navegação segura
- Noções gerais de tecnologia

 📊 Pacote Office
- Word (documentos profissionais)
- Excel (planilhas e cálculos)
- PowerPoint (apresentações)

 🎨 Design Gráfico
- Adobe Photoshop (edição de imagens)
- Adobe Illustrator (criação de artes)

 💻 Programação (nível inicial)
- Lógica de programação
- Programação com Scratch
- Noções de funcionamento de aplicativos

 🌐 Criação de Sites
- HTML e CSS
- Ferramentas como Dreamweaver

 📱 Desenvolvimento e Apps (básico)
- Introdução ao desenvolvimento de aplicativos
- Ferramentas de prototipagem (Adobe XD)

---

  Metodologia do curso

- Aulas práticas desde o início
- Projetos para fixação do conteúdo
- Equilíbrio entre teoria e prática

---

  Habilidades adquiridas

- Uso profissional do computador
- Criação de documentos, planilhas e apresentações
- Edição de imagens
- Desenvolvimento de páginas web simples
- Base em lógica de programação

---

  Observação

Este curso possui foco introdutório/intermediário, oferecendo uma base sólida para quem está começando na área de tecnologia — um verdadeiro "primeiro passo" na jornada.

 💻 Tecnologias que já explorei

- 🌐 **HTML** – Estrutura de páginas web  
- ⚡ **JavaScript** – Noções de programação web  
- 🎨 **CSS** – Estilização de sites  
- 🖥️ **VirtualBox** – Virtualização de sistemas  
- 💻 **Sistemas Operacionais** – Windows e Linux
---

- Contato

- 📧 Email: miguelmunhenhomgl@gmail.com
- 💼 LinkedIn: https://www.linkedin.com/in/miguel-soledade-delurdes-1266693b0/

---

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/GabrielaZanetti/GabrielaZanetti/output/github-contribution-grid-snake-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/GabrielaZanetti/GabrielaZanetti/output/github-contribution-grid-snake.svg">
  <img alt="github contribution grid snake animation" src="https://raw.githubusercontent.com/GabrielaZanetti/GabrielaZanetti/output/github-contribution-grid-snake.svg">
</picture>

---

 🎧 Interesses pessoais

Quando não estou estudando ou praticando tecnologia, gosto de ouvir música. Minhas bandas favoritas são:

-  Deftones
-  RadioHead
-  The smiths
-  Queen  
-  Nirvana  
-  Charlie Brown Jr  
-  Guns N' Roses  

---

 📈 Objetivo

Assim como um verdadeiro usuário de Stand, meu objetivo é evoluir constantemente, adquirir novas habilidades e me tornar um profissional cada vez mais completo na área de tecnologia.

---

 Obrigado por visitar meu perfil!
 <img width=100% src="https://capsule-render.vercel.app/api?type=waving&color=683086&height=150&section=footer"/>
